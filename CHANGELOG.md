...
**Bug Fixes:**
- **Error messages are readable again** - The error toast set its background and text from two colour tokens that were never defined anywhere, so it painted transparent with inherited text. An error was reported invisibly on top of your bookmarks. Both tokens are now defined.
...
- **Wording no longer says "Snippet" everywhere** - The settings, buttons and messages now say cloud, because where your bookmarks live is a detail that can change. The snippet-specific screens keep their wording, since that is still what they do.
...
- **The sync settings offer a way in when nothing is connected** - A device holding a token but connected to no store had no setup button at all, leaving the settings with Disconnect and Cancel and no route to a store. It now shows a Set Up Bookmark Sync button whenever nothing is connected.
- **The share window no longer refers to your Snippet** - The list of changes waiting for your approval still said "Remove 2 bookmarks from your Snippet". It now says cloud bookmarks, like everywhere else.
- **Connecting for the first time no longer offers to delete your own bookmarks** - When a bookmark is on this device but not in the cloud, BMZ decides what to do by checking whether it watched you add it. If it did, the bookmark is sent up. If it has no record, it assumes another device deleted it and asks your permission first. Bookmarks brought in by an import carry no such record, and neither does a device connecting for the very first time, so your own bookmarks looked like things to delete. Everything already on the device is now treated as yours at the moment you first connect.
...
- **A failed read no longer disconnects you** - If BMZ could not read your bookmarks from the cloud, it forgot which store you were using and dropped you back into setup. One bad connection, one expired token or one bad address was enough. It now keeps the connection and reports the failure instead.
- **Joining a repository no longer writes over it while a decision is pending** - When merging found something it would have to remove or overwrite, it stopped and asked, and then sent this device's bookmarks up anyway, undoing exactly what it had just protected. It now waits for your answer.
...
**Changes:**
- **BMZ opens roughly twice as fast** - Opening the site used to wait for a full check against your snippet before it would show you anything, even though your bookmarks were already stored on this device and were what the list draws from. The list now appears straight away and the cloud check runs behind it, so anything waiting in your snippet arrives a moment later on its own. If a sync needs your approval, that card now appears just after the list rather than with it.
- **Sharing a link to BMZ opens faster** - The share window checked your snippet twice, once before the window could appear and once after, and both checks did the same work. It also built your entire bookmark list behind a window that completely covers it, along with the link and safety data that share windows never use. It now checks once and prepares only what the share form actually needs: your folders, and whether the link is already saved.
